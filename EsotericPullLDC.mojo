...
from src.lbm import (
                    ...
                    LBM_Grid,LBM_Config,
                    get_D2Q9,set_exterior_walls,calculate_esoteric_rho_and_velocity,
                    UnitSystem
                    )
...
def main() raises:
    ...
    comptime get_u_and_rho = calculate_esoteric_rho_and_velocity[False,f_layout,bc_layout,flag_layout,density_layout,velocity_layout,grid,config]
    ...
    for step in range(MAX_ITERS):
        ctx.enqueue_function(LBM_even_step,f.gpu(),bc.gpu().as_immut(),flags.gpu().as_immut(),tau,grid_dim = GRID_DIM,block_dim = BLOCK_SHAPE)
        ctx.enqueue_function(LBM_odd_step,f.gpu(),bc.gpu().as_immut(),flags.gpu().as_immut(),tau,grid_dim = GRID_DIM,block_dim = BLOCK_SHAPE)
