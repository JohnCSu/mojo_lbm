...
def benchmark_func[
    ...
    ctx.synchronize()
    #Compile Functions
    comptime LBM_kernel_ref_2172 = LBM_kernel[type_of(f_layout),type_of(bc_layout),type_of(flag_layout),grid]
    var LBM_func = ctx.compile_function[LBM_kernel_ref_2172]()
    ctx.synchronize()
