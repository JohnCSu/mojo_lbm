...
def interpolated_BB[
    ...
    config:LBM_Config,
        ...
        f_out:TileTensor[config.set_f_dtype(grid.float_dtype),FLayoutType,ImmutAnyOrigin],
        flags:TileTensor[DType.uint8,FlagLayoutType,ImmutAnyOrigin],
        links_fluid_to_solid:TileTensor[grid.float_dtype,type_of(rowMajor2D[grid.int_dtype]()),MutAnyOrigin],
        links_q:TileTensor[grid.int_dtype,type_of(rowMajor1D[grid.int_dtype]()),MutAnyOrigin],
    ...
    if tid < links_q.layout.size():

        fluid_idx = links_fluid_to_solid[tid,0]
        solid_direcion = links_fluid_to_solid[tid,1]
