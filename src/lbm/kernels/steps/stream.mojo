...
#         comptime assert is_even_time_step is not None, 'If lbm_method is set to esoteric_pull, is_even_time_step must be defined'
#         f_vec = esoteric_pull_load_f_vec[float_dtype,is_even_time_step.value(),use_float16c](f,index,grid_shape,directions)
...
#         set_adjacent_flags[directions](pull_flags,flags,index,grid_shape)
#         f_vec = double_buffer_pull_load_f_vec[float_dtype,use_float16c](f,pull_flags,index,grid_shape,directions,opposite_indices,)
#     else:
#         comptime assert False, 'lbm_method not valid'