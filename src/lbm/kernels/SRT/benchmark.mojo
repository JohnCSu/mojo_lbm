...
def run_benchmark[
    ...
    comptime if not config.DDF_shift:
        f.fill(Scalar[f_dtype](1.0 / Float32(Q)))
        f_out.fill(Scalar[f_dtype](1.0 / Float32(Q)))
    else:
        f.fill(0)
        f_out.fill(0)

    set_exterior_walls[grid, config](
        flags.cpu(), bc.cpu(), "+Y", SOLID_NODE, [U, 0, 0], 1.0
    )
    set_exterior_walls[grid, config](
        flags.cpu(), bc.cpu(), "-Y", SOLID_NODE, [0, 0, 0], 1.0
    )
    set_exterior_walls[grid, config](
        flags.cpu(), bc.cpu(), "-X", SOLID_NODE, [0, 0, 0], 1.0
    )
    set_exterior_walls[grid, config](
        flags.cpu(), bc.cpu(), "+X", SOLID_NODE, [0, 0, 0], 1.0
    )
